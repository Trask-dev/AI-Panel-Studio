...
class TestSSEManager:
    ...
    async def test_cross_discussion_isolation(self):
        ...
        await manager.push(SSEEvent(
            type="snapshot_update",
            payload={"discussion_id": "disc-1", "guest_id": "g1"},
        ))

        # q1 收到
        data = await asyncio.wait_for(q1.get(), timeout=1.0)
    ...
    async def test_multiple_subscribers_same_discussion(self):
        ...
        data_a = await asyncio.wait_for(q_a.get(), timeout=1.0)
        data_b = await asyncio.wait_for(q_b.get(), timeout=1.0)
